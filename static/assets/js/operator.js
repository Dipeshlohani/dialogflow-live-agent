$(function () {
  ...
  $('form').submit(function (e) {
    ...
    socket.emit(
      'operator message',
      messageObject(currentTab.customerId, messageText)
    );
    $('#m').val('');
    return false;
  });
  ...
  var createNewCustomerTab = function (customerId) {
    ...
    // if (customerId) {
    //   $('#chatWindows')
    //     .append(`<div onClick="handleWindowClick('${customerId}');" class="avn_single_chat_items chat_active">
    //   <div class="avn_chat_logo">
    // 	  <img src="/assets/images/img/chat1.png" alt="">
    //   </div>
    //   <div class="avn_chat_title">
    // 	  <h3>${customerId}</h3>
    // 	  <p>INBOX</p>
    //   </div>
    // </div>`);
  ...
  var notifyOperatorRequest = function (customerId) {
    if (!connectedCustomers[customerId]) {
      console.log(
        'Received operator request from unknown customer id: ' + customerId
      );
      return;
    }
  ...
  var notifyCustomerDisconnected = function (customerId) {
    if (!connectedCustomers[customerId]) {
      console.log(
        'Received disconnect notification for unknown customer id: ' +
          customerId
      );
      return;
    }
    ...
    connectedCustomers[customerId].window.append(
      $('<li class="customer-message">').text('--- Customer disconnected ---')
    );
  };
  ...
  var notifySystemError = function (error) {
	...
	<div class="msg_right_items">
		<div class="single_send_msg">
			<p style="color:red;">${errorText}...</p>
		</div>
	</div>
	</li>`);
    // currentTab.window.append(
    //   $('<li class="operator-error collection-item">').text(errorText)
    // );
  };
  ...
  var receivedOperatorMessage = function (msg) {
    ...
    if (!customer) {
      console.log(
        'Received operator message to unknown customer id: ' +
          JSON.stringify(msg)
      );
      return;
    }
    $('#message-list').append(`<li class="right_send_msg">
	<div class="msg_right_items">
		<div class="single_send_msg">
			<p>${msg.utterance}</p>
		</div>
		<span class="send_time">${currentTime}</span>
	</div>
	</li>`);
    // customer.window.append(
    //   $('<li class="operator-message collection-item">').text(msg.utterance)
    // );
  };
  ...
  var receivedCustomerMessage = function (msg) {
    ...
    if (!connectedCustomers[msg.customerId]) {
      console.log(
        'Received message for unknown customer id: ' + JSON.stringify(msg)
      );
      return;
    }
    ...
    if (!msg.isAgentResponse) {
      $('#message-list').append(`  <li class="left_send_msg">
		<div class="c_logo">
			<img src="assets/images/img/chat1.png" alt="">
		</div>
		<div class="msg_left_items">
			<div class="single_send_msg">
				<p>${msg.utterance}</p>
			</div>
			<span class="send_time">${currentTime}</span>
    ...
    // prefix = 'Robin Sams';
    // connectedCustomers[msg.customerId].window.append(
    //   $('<li class="customer-message"><br/>')
...
function handleWindowClick(customerId) {
  console.log('OKAY', customerId);
  $('#current-customer').html(` <div class="avn_cname_logo">
  <img src="/assets/images/img/chat1.png" alt="">
</div>
<div class="avn_chat_title">
  <h3>${customerId}</h3>
